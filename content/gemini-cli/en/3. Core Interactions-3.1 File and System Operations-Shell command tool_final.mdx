...
description: "Get an in-depth look at the Gemini CLI's run_shell_command tool, including interactive commands, command limit configuration, and security considerations."
tool: "gemini-cli"
...
- **Background processes:** When a command is run in background with `&`, the
...
Security note for `excludeTools`: Command-specific restrictions in `excludeTools`
for `run_shell_command` are based on simple string matching and can be easily
