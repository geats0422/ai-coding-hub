---
title: "Extensions Management"
description: "Learn how to install, manage, and create Gemini CLI extensions, and learn about extension formats, configurations, and best practices."
tool: "gemini-cli"
slug: "extensions-management"
locale: "en"
---

import { AdPlaceholder } from '@/components/AdPlaceholder'
import { Callout } from '@/components/Callout'

# Gemini CLI extensions

Gemini CLI extensions package prompts, MCP servers, custom commands, hooks,
sub-agents, and agent skills into a familiar and user-friendly format. With
extensions, you can expand the capabilities of Gemini CLI and share those
capabilities with others. They are designed to be easily installable and
shareable.

To see examples of extensions, you can browse a gallery of
[Gemini CLI extensions](https://geminicli.com/extensions/browse/).

<AdPlaceholder />

## Managing extensions

You can verify your installed extensions and their status using the interactive
command:

```bas
/extensions list
```

or in noninteractive mode:

```bas
gemini extensions list
```

## Installation

To install a real extension, you can use the `extensions install` command with a
GitHub repository URL in noninteractive mode. For example:

```bas
gemini extensions install https://github.com/gemini-cli-extensions/workspace
```

## Next steps

- [Writing extensions](/docs/gemini/extensions-management): Learn how to create your first
  extension.
- [Extensions reference](/docs/gemini/extensions-management): Deeply understand the extension format,
  commands, and configuration.
- [Best practices](/docs/gemini/extensions-management): Learn strategies for building great
  extensions.
- [Extensions releasing](/docs/gemini/extensions-management): Learn how to share your extensions with
  the world.

<AdPlaceholder />
