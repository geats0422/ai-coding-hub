...
const mdxModulesEn = import.meta.glob('../content/codex/en/*.mdx', {
  eager: true,
}) as Record<string, MdxModule>;

const mdxModulesZh = import.meta.glob('../content/codex/zh/*.mdx', {
  eager: true,
}) as Record<string, MdxModule>;
...
export const CodexDocs: React.FC = () => {
  const { language } = useLanguage();
  const uncategorizedLabel = language === 'zh' ? '未分类' : 'Uncategorized';
  const docsLabel = language === 'zh' ? 'Codex 文档' : 'Codex Docs';
  const emptyStateLabel =
    language === 'zh'
      ? '未找到可加载的文档文件，请检查 `content/codex/zh` 或 `content/codex/en`。'
      : 'No loadable docs found. Please check `content/codex/zh` or `content/codex/en`.';
  ...
  const docs = useMemo<CodexDocItem[]>(() => {
  ...
  useEffect(() => {
    ...
    const canonicalUrl = `https://www.aicodinghub.dev/docs/codex/${activeDoc.slug}`;
  ...
  if (docs.length === 0) {
    return (
      <main className="flex-1 px-6 py-10">
        <h1 className="text-2xl font-bold mb-2">Codex</h1>
  ...
  if (!hasSlugMatch) {
    return <Navigate to={`/docs/codex/${docs[0].slug}`} replace />;
  }
