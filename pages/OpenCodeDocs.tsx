...
const mdxModulesEn = import.meta.glob('../content/opencode/en/*.mdx', {
  eager: true,
}) as Record<string, MdxModule>;

const mdxModulesZh = import.meta.glob('../content/opencode/zh/*.mdx', {
  eager: true,
}) as Record<string, MdxModule>;
...
export const OpenCodeDocs: React.FC = () => {
  const { language } = useLanguage();
  const uncategorizedLabel = language === 'zh' ? '未分类' : 'Uncategorized';
  const docsLabel = language === 'zh' ? 'OpenCode 文档' : 'OpenCode Docs';
  const emptyStateLabel =
    language === 'zh'
      ? '未找到可加载的文档文件，请检查 `content/opencode/zh` 或 `content/opencode/en` 下的 `.mdx` 文件。'
      : 'No loadable docs found. Please check `.mdx` files under `content/opencode/zh` or `content/opencode/en`.';
  ...
  const docs = useMemo<OpenCodeDocItem[]>(() => {
  ...
  useEffect(() => {
    ...
    updateSeo({
      ...
      canonicalUrl: `https://www.aicodinghub.dev/docs/opencode/${activeDoc.slug}`,
    });
  }, [activeDoc]);
  ...
  if (docs.length === 0) {
    return (
      <main className="flex-1 px-6 py-10">
        <h1 className="text-2xl font-bold mb-2">OpenCode</h1>
